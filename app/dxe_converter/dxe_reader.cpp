...
ACE_Time_Value DXEReader::get_ace_time(const std::string& ts_, bool& same_day_)
{
	...
	long usec = 0;
	if (ts_.length() > 21)	
		usec = atoi(ts_.substr(21).c_str())*1000;
	TCDateTime tcdt(ttm);
...
bool DXEReader::make_key(const std::string& s_, std::string& key_, const ACE_Time_Value& tv_)
{
	...
	_base_symbol = LFSecurityMaster::instance()->CQG_SEC;
	std::string ss = s_.substr(0, 2);
	if (ss == LFSecurityMaster::instance()->DX_EXCH)
	{
		_base_symbol += LFSecurityMaster::instance()->DXE;
		front = LFSecurityMaster::instance()->get_dxe(d);
		tomorrow = LFSecurityMaster::instance()->get_dxe(t);
		_ticker = t_DXE;
	}
	else if (ss == LFSecurityMaster::instance()->QO_EXCH)
	{
		_base_symbol += LFSecurityMaster::instance()->QO;
		front = LFSecurityMaster::instance()->get_brn(d);
		tomorrow = LFSecurityMaster::instance()->get_brn(t);
		_ticker = t_BRN;
	}
	else if (ss == LFSecurityMaster::instance()->RF_EXCH)
	{
		_base_symbol += LFSecurityMaster::instance()->RFE;
		front = LFSecurityMaster::instance()->get_rfe(d);
		tomorrow = LFSecurityMaster::instance()->get_rfe(t);
		_ticker = t_RFE;
	}
	else if (ss == LFSecurityMaster::instance()->ETQO_EXCH)
	{
		_base_symbol += LFSecurityMaster::instance()->ETQO;
		front = LFSecurityMaster::instance()->get_etqo(d);
		tomorrow = LFSecurityMaster::instance()->get_etqo(t);
		_ticker = t_ETQO;
	}
	else
	{
