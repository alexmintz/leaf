...
void CClrButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (state & ODS_DISABLED)
	{
		DrawButtonText(dc, offset_text_rect, CT2A(buffer), CLR_BTN_WHITE);
		DrawButtonText(dc, text_rect, CT2A(buffer), CLR_BTN_DGREY);
    }
	else
	{
		if (state & ODS_SELECTED)
		{
			DrawButtonText(dc, offset_text_rect, CT2A(buffer), text_colour);
		}
		else
		{
			DrawButtonText(dc, text_rect, CT2A(buffer), text_colour);
		}
	}
...
//	dc->DrawText(Buf, strlen(Buf), r, DT_CENTER | DT_VCENTER | DT_CALCRECT);

	dc->DrawText(CA2T(Buf), (int)(strlen(Buf)), r, DT_CENTER | DT_VCENTER);
