...
//			subscribe_input(this, &LFStrategy::process_stratpos, master_key, "strat_pos", true);
			BMTS<LFInfo_Position>::instance()->_ignore_empty = true;
			subscribe_input(this, &LFStrategy::process_stratpos, master_key, "md_in", false, hwm, lwm);
...
void LFStrategy::thr_close()
{
	...
	if (Z::get_setting_bool("LFStrategy:LFInfo_Position:in", false))
		unsubscribe_input<LFInfo_Position>("md_in");
...
//--> Signals Factory
	...
	// LCY
	sptr.reset(new LFSignal_LCY(*this, _s._f_1));
...
void LFStrategy::save_position()
{
	...
	if(l_stream.good())
	{
		...
		std::string account_id = _pos->account();
		...
		l_stream << "LFStrategy:cqg_account_id" << "\t" << account_id << std::endl;
		l_stream << "LFStrategy:strategy_id" << "\t" << _key << std::endl;
		LFAccount acc =  LFSecurityMaster::instance()->account_info(account_id);
		l_stream << "LFAccount:" << acc.get_name() << "\t" << acc.get_pct() << std::endl;
		for (LFAccount::SubAccMap::const_iterator i = acc.it_begin(); i != acc.it_end(); ++i)
			l_stream << "LFAccount:" << acc.get_name() << ":" << i->second.get_name() << "\t" << i->second.get_pct() << std::endl;
...
//					_pos->new_order(LFSide::contra(pd._side), q, LF::otMarket, -1);
	...
	else if(dd._st_action != LF::satUNKNOWN && _trading_signal.get() != 0)
	{
...
//AK check
	...
	pd._lcy_trends = _s._f_1._lcy_trends;
