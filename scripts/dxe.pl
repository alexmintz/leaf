...
{
	@estuff = ($_[0], $_[1]);
    	print LOG "Running dxe_converter @estuff \n";
...
#       (my $basename = $_[0]) =~ s!^.*/!!;
	...
	#unzip everything first
        ...
        foreach $_ (@file_list)
    	{
		print LOG "starting... ($counter of $list_size) dxe_converter -d $_ \n";
                if($dry == 0)
                {
       			my $pid = &runit("-d",  $_);
			$counter++;
	...
	my $log_file = cwd()."/dxe_script.log";
	my $ret = GetOptions("d"=>\$dry,
					"r=s"=>\$dir_root,
					"p:i"=>\$max_pid,
					"l:s"=>\$log_file);

	if($ret == 0 || !$dir_root) 		{die "Usage: dxe.pl -r {root} [ -p{max pid} | -l{log_file} |-d ]\n";}
  	unless( -d $dir_root)	{die("root directory: ${dir_root} does not exist!, stopped\n");}

	open (LOG, ">>$log_file");
	LOG->autoflush(1);
 	print "dxe.pl -r $dir_root -p $max_pid -l $log_file -d $dry\n";
	$SIG{INT} = \&zap;
